...
Output:
  ...
  run_all_test.sh        — master parallel runner (xargs -P 4)
"""
...
def main():
    ...
    interp_cases, interp_rows = create_cases(interp_pts, "interp")

    print("\nCreating extrapolation cases:")
    extrap_cases, extrap_rows = create_cases(extrap_pts, "extrap")
...
set -e
echo "Launching {len(all_cases)} test cases, 4 in parallel..."
printf '%s\\n' {" ".join(all_cases)} | xargs -P 4 -I {{}} bash "{TEST_DIR}/{{}}/run_case.sh"
