...
def main():
    ...
    Z = np.load(os.path.join(MODEL_DIR_AE, f"latent_d{d}.npy"))

    param_rows = load_training_params()
    param_dict = {row[0]: (row[1], row[2]) for row in param_rows}
    X_params = np.array([param_dict[c] for c in cases])
    ...
    for k in range(n):
        ...
        mlp_loo.fit(X_scaled[mask], Z[mask])
        Z_val_pred = mlp_loo.predict(X_scaled[[k]])
        loo_mses.append(float(np.mean((Z[[k]] - Z_val_pred) ** 2)))
        if (k + 1) % 10 == 0:
            print(f"  LOO {k+1}/{n}")
    ...
    ax.semilogy(mlp.loss_curve_, color="steelblue", lw=1.5)
    ax.set_xlabel("Epoch")
