...
Also generates:
  run_all_training.sh  — master parallel runner (xargs -P 30)
...
# Import toolpath generator from project folder
...
# Parameter grids
HATCH_VALS  = np.linspace(50e-6, 150e-6, 10)   # m
SPEED_VALS  = np.linspace(0.8, 1.6, 10)         # m/s

# Fixed toolpath parameters
FIXED_TP = dict(
    center_x=1e-3,
    center_y=1e-3,
    start_z=0.0006975,
...
def get_toolpath_end_time(crs_path):
    """Return the last positive time entry from a .crs file."""
    t_last = 0.0
...
def make_input_param(base_txt, case_name, toolpath_abs, timax):
    """Return modified input_param.txt text for this case."""
    txt = base_txt
    # case_name
...
def main():
    ...
    for i, hs in enumerate(HATCH_VALS):
        for j, ss in enumerate(SPEED_VALS):
            ...
            # Write run_case.sh
            run_sh = make_run_script(case_dir, case_name)
    ...
    # Write run_all_training.sh at project root
    run_all_path = os.path.join(PROJECT_DIR, "run_all_training.sh")
    with open(run_all_path, "w") as f:
        f.write(f"""#!/bin/bash
# Run all 100 training simulations in parallel (30 at a time, 1 OMP thread each)
