...
# ---------------------------------------------------------------------------
# VTK reader (same as in train_autoencoder.py)
# ---------------------------------------------------------------------------
def read_defect_vtk(filepath):
    ...
    while True:
        ...
        if line.startswith("DIMENSIONS"):
            ...
            return arr.reshape(dims)   # (nz, ny, nx)
    raise ValueError(f"Could not find 'SCALARS defect' in {filepath}")


def defect_vtk_to_2d(filepath):
    """Load defect VTK and return Z-averaged 2D array (ny, nx) flat."""
    arr3d = read_defect_vtk(filepath)
    return arr3d.mean(axis=0)   # (ny, nx)


# ---------------------------------------------------------------------------
# Porosity fractions from defect array (matching mod_defect.f90 definition)
# Equal-volume cells: fractions = sums / total_voxel_count
# ---------------------------------------------------------------------------
def compute_porosity(arr2d):
    """Compute LOF, keyhole, total porosity fractions from 2D defect array."""
    n = arr2d.size
...
def load_models():
    ...
    # X_flat needed for PCA reconstruction
    X_flat = np.load(os.path.join(MODEL_DIR_AE, "X_flat.npy"))
    shape_2d = None  # will be inferred from test VTK

    return pca, mlp, scaler, d, X_flat


# ---------------------------------------------------------------------------
# Predict defect field from process parameters
# ---------------------------------------------------------------------------
def predict_defect_2d(hs, ss, pca, mlp, scaler):
    """Return predicted 2D defect array (flat vector) for given process params."""
    X_in = scaler.transform([[hs, ss]])
    z_pred = mlp.predict(X_in)         # (1, d)
    x_recon = pca.inverse_transform(z_pred)  # (1, n_features)
    return x_recon[0]                   # (n_features,)
...
def evaluate_case(entry, pca, mlp, scaler):
    ...
    hs, ss    = entry["hs"], entry["ss"]

    vtk_path = os.path.join(TEST_DIR, case_name, "result",
                            f"{case_name}_defect.vtk")
    if not os.path.exists(vtk_path):
        return None

    true_2d   = defect_vtk_to_2d(vtk_path)   # (ny, nx)
    true_flat = true_2d.flatten()

    pred_flat = predict_defect_2d(hs, ss, pca, mlp, scaler)
...
def plot_field_comparison(results, title_prefix, filename, n_show=3):
    """Side-by-side 2D slices: true / predicted / difference for up to n_show cases."""
    valid = [r for r in results if r is not None][:n_show]
    if not valid:
        print(f"No valid results for {filename}")
        return

    # Compute common color range (1st/99th percentile across shown cases)
...
def plot_porosity_parity(interp_results, extrap_results):
    """Parity plots for LOF, keyhole, total porosity fractions."""
...
def plot_error_summary(interp_results, extrap_results):
    """Five side-by-side bar charts: mean interp vs extrap for each metric."""
    metrics = [
        ("mse",     "Field MSE"),
        ("r2",      "Field R²"),
...
def main():
    ...
    pca, mlp, scaler, d, X_flat = load_models()
    ...
    print("\n=== Summary ===")
    headers = ["Metric", "Interpolation", "Extrapolation"]
    rows = [
