   ...
   ! real(8) massdiffusivity(i,j,k) ! = rou*D=10000*1e-7

	do k=1,nz
	do j=1,ny
	do i=1,nx
    massdiffusivity(i,j,k)=massdiffl
    if(temp_hyw(i,j,k).le.tsolidmatrix(i,j,k))massdiffusivity(i,j,k)=1e-11
	enddo
	enddo
	enddo
...
subroutine source_species
    ...
    real(8) kp ! Equilibrium partition coefficient
    kp=0.16
...
!	if(.not.steady.and.ipoweroff.ge.1) then
!		variable1=dens/delt
!	do k=2,nkm1
!	do j=2,njm1
!	do i=2,nim1
...
!-----under-relaxation---
		ap(i,j,k)=ap(i,j,k)/urfh             ! for species transport, this coefficient (urfspecies) can be changed accordingly.
